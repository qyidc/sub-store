/**
 * =================================================================================
 * 欢迎来到 Cloudflare Workers! (功能整合最终版)
 * =================================================================================
 *
 * 【架构定型】:
 * 1. 【功能完备】: 本脚本集成了所有已开发的功能：多协议解析、三类型链接生成、凭码提取、自定义有效期。
 * 2. 【安全可靠】: 内置了Cloudflare Turnstile人机验证和CORS跨域请求处理。
 * 3. 【代码稳健】: 这是为当前所有功能设计的最终稳定版后端代码。
 */
...
router.post(/^\/convert$/, async ({ request, env }) => {
	try {
		...
		if (!subscription_data) {
			return new Response('订阅数据不能为空。', { status: 400, headers: corsHeaders });
		}
...
function parseShadowsocks(link) {try{const url = new URL(link);const b64part = url.href.substring(5, url.href.indexOf('@'));const decoded = atob(b64part);const [cipher, password] = decoded.split(':');const [server, port] = url.hostname.split(':');const name = decodeURIComponent(url.hash).substring(1) || `${url.hostname}:${url.port}`; return {name: name, type: 'ss', server: url.hostname, port: parseInt(url.port, 10), cipher: cipher, password: password, udp: true};}catch(e){try{let newLink = "ss://" + atob(link.substring(5)); return parseShadowsocks(newLink);} catch(e2){return null;}}}
